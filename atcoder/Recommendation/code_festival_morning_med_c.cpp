...
struct mat {
    ...
    void pow_init(){
        ...
        rep(i, 1, MAX_SIZE){
            v[i] = v[i-1]*v[i-1];
            // 誤差対策でこの問題専用に追記
            v[i].m[0][0] = v[i].m[0][0]/(v[i].m[0][0]+v[i].m[0][1]);
            v[i].m[0][1] = v[i].m[0][1]/(v[i].m[0][0]+v[i].m[0][1]);
            v[i].m[1][0] = v[i].m[1][0]/(v[i].m[1][0]+v[i].m[1][1]);
            v[i].m[1][1] = v[i].m[1][1]/(v[i].m[1][0]+v[i].m[1][1]);
        }
    }
