...
int main()
{
    ...
    Matrix<XOR_AND_semiring> mat(k,k);
    rep(i,0,k) mat[0][i] = c[i];
    rep(i,1,k) mat[i][i-1] = one002();

    auto arr = mat.pow(m-k);
    Matrix<XOR_AND_semiring> a2(k,1);
    rep(i,0,k) a2[i][0] = a[k-i-1];
    auto ans = arr * a2;
    cout << ans[0][0] << endl;
