...
int main()
{
    ...
    Potential_union_find<ll> ufx(n);
    Potential_union_find<ll> ufy(n);
    vector<ll> ansx(n);
    vector<ll> ansy(n);
    rep(i,0,m){
        ll a,b,x,y;
        cin>>a>>b>>x>>y;
        a--;b--;
        assert(ufx.set_diff(a,b,x));
        assert(ufy.set_diff(a,b,y));
    }

    rep(i,0,n){
        ll x = ufx.get_diff(0, i);
        ll y = ufy.get_diff(0, i);
        if (x==INF){
